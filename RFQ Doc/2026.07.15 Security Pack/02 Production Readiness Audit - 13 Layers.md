...
|---|---|---|---|
| 1 | Authentication & session | ❌ Fail | Azure AD and ADFS SSO, SAML 2.0, MFA enforced |
...
| 5 | Input validation & injection | ✅ Pass | Maintain, and add schema validation at the API boundary |
...
| 10 | Deployment & CI/CD | 🟡 Partial | GitHub Enterprise and Azure DevOps, gated pipelines, no manual prod access |
| 11 | Logging, audit & monitoring | 🟡 Partial | App Insights and Azure Sentinel SIEM (SoW-mandated) |
...
The `getSession()` helper in `src/lib/auth.ts` returns `DEFAULT_USER`, whose role is
`governance_lead`, when no session cookie is present. Any anonymous caller therefore holds
the highest privilege.

*Reproduced:* `curl -X PATCH /api/v1/publish/targets/1 -d '{"status":"queued"}'` with no
cookie returns `200` and mutates data.

*Why it exists:* demo convenience, so reviewers land on the app already signed in.
*Production fix:* delete the fallback. Unauthenticated becomes `401`. A session is
established only via Azure AD or ADFS. **Blocking for any deployment beyond localhost.**
...
no token, and no IdP. Impersonation is a single request.

*Why it exists:* the demo role switcher.
*Production fix:* remove the endpoint entirely and replace it with a SAML 2.0 assertion
from ADFS or Azure AD per SoW §3.4. **Blocking.**
...
*Fix:* fail fast on startup if `JWT_SECRET` is absent, source it from Key Vault, and use a
random value of 256 bits or more.

**H-2: Vulnerable framework version**
`next@14.2.5` carries **high** severity advisories: DoS via Server Components and the Image
Optimizer, XSS in App Router with CSP nonces, cache poisoning of RSC responses, request
smuggling in rewrites, and SSRF via WebSocket upgrades. `postcss` carries a moderate XSS
advisory.
*Evidence:* `npm audit --omit=dev` reports 2 vulnerabilities, 1 high and 1 moderate.
*Fix:* upgrade Next.js and re-test the demo arc. This is a **major version** upgrade, so
schedule it rather than hot-patching it the night before a demo.
...
*Fix:* set `secure: true` in any non-local environment and enforce HTTPS with HSTS at the
edge.

**H-4: No transport encryption**
The sample serves plaintext HTTP on `localhost:3000`, and Postgres runs with `trust` auth,
requiring no password locally.
*Fix:* terminate TLS at Azure Front Door. Run Postgres over TLS with password or
managed-identity auth behind a private endpoint.

### 🟡 MEDIUM

**M-1: No security response headers.** `next.config.mjs` is empty, so there is no CSP,
HSTS, X-Content-Type-Options, X-Frame-Options or Referrer-Policy. *Fix:* add a `headers()`
block, or enforce at Front Door.

**M-2: No rate limiting or bot protection** on any route, including the model-backed AI
endpoints, which are a cost-amplification vector once a key is configured. *Fix:* WAF rate
rules plus per-key throttling.

**M-3: No backup or restore procedure.** `npm run db:reset` destroys and re-seeds. There is
no backup, no defined RPO/RTO, and no restore drill. *Fix:* Azure PITR plus a documented,
rehearsed restore.

**M-4: API input not schema-validated.** Handlers hand-check fields, so malformed bodies
rely on Postgres constraints as the backstop. *Fix:* validate at the boundary (for example
with Zod) and return `400` consistently.
...
hole today. *Fix:* anti-CSRF tokens, or rely on IdP bearer tokens once C-1 and C-2 are
closed.
...
- **P-1: No SQL injection surface.** Every query goes through `pg` with parameterised
  ...
  **zero** hits. User-supplied filters (`class`, `hierarchy`, `q`) are bound, never
  concatenated.
- **P-2: Secrets are not in version control.** `.env.local` is gitignored and no `.env`
  file is tracked. Confirmed against the git index.
- **P-3: Immutable audit trail exists by design.** Every mutation writes `audit_log` with
  actor, action, entity, before and after values, and timestamp. This is the control
  SoW §3.1.4 asks for, built in from S2 rather than retrofitted.
...
|---|---|---|---|---|
| C-1 | Remove `DEFAULT_USER` fallback, return `401` | ~1 h | Eng | Before any shared/hosted environment |
| C-2 | Remove `/auth/switch`, wire ADFS/Azure AD SAML | 3 to 5 d | Eng | Before pilot |
| H-1 | Fail fast on missing `JWT_SECRET`, use Key Vault | ~2 h | Eng | Before pilot |
| H-2 | Upgrade Next.js, re-verify demo arc | 0.5 to 2 d | Eng | Before pilot |
| H-3 | `secure: true` plus HSTS | ~1 h | Eng | Before pilot |
| H-4 | TLS end to end, Postgres TLS and auth | 1 to 2 d | Eng/Infra | Before pilot |
| M-1 | Security headers and CSP | ~4 h | Eng | Before pilot |
| M-2 | WAF rate limiting | ~1 d | Infra | Before pilot |
| M-3 | Backups plus documented, drilled restore | 2 to 3 d | Infra | Before go-live |
| M-4 | Boundary schema validation | 1 to 2 d | Eng | Before go-live |
...
listed as Critical because if the demo is ever exposed beyond localhost, for example on a
shared staging URL for Maaden to click through, they become live vulnerabilities that hour
one of any real assessment will find.

## Re-audit

This document is audit **#1**. Per XD House practice the sequence is: audit, then
remediate, then external penetration test, then re-audit.
