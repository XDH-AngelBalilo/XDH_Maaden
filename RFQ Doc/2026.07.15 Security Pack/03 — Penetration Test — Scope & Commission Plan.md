...
**In scope**
- Web application (7 screens + AI Assistant), authenticated and unauthenticated surfaces
- REST API `/api/v1/*` — all methods, including the OpenAPI-documented surface
- **Authorization / privilege escalation across the 4 roles** — the highest-value area
  given the demo's history (C-1/C-2). Explicitly test: can a Viewer mutate? can an Engineer
  approve a template or publish? can role claims be forged?
- Session management: JWT handling, cookie flags, fixation, replay
- Injection: SQLi, XSS, SSRF, template/JSON injection
...
- Maaden's own downstream systems (Aconex, P6, SAP, AVEVA PI) — connector *interfaces*
  only, not the systems themselves
...
**Standard:** OWASP ASVS / OWASP Top 10 + API Security Top 10 as the baseline.
...
| Duration | `[XDH TO CONFIRM — typically 5–10 days for this scope]` |
| Budget | `[XDH TO CONFIRM]` |
| Deliverable | Findings report with severities + evidence, retest letter after fixes |
