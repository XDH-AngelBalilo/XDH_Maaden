...
`[XDH TO CONFIRM]` — whether any sub-processor (e.g. model APIs used by the optional AI
layer) receives data, and if so what, where, and under what terms. State this plainly; do
...
| | Control |
|---|---|
| **In transit** | TLS 1.2+ for all traffic, browser→app and app→database. HSTS enforced at the edge. |
...
- **Identity:** Azure AD + ADFS SSO via SAML 2.0 — Maaden's own identity provider is the
...
- **Audit:** every change writes an immutable audit record — who, what, before/after value,
  timestamp (SoW §3.1.4). Approved data cannot be silently edited; a change creates a new
  revision and forces re-validation.
...
| Re-audit confirming fixes held | After remediation + pen test | ⏳ Pending |
...
(e.g. ISO 27001, SOC 2). **If none are held, say so and state the alignment instead** —
e.g. "we align to ISO 27001 practice; we are not certified." An unsupportable claim here
is the fastest way to fail a security review.
