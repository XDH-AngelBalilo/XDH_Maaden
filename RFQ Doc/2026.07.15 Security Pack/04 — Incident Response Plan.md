...
> This draft is structured to be signed, dated, and produced on request — but every
...
supporting infrastructure — including the production environment, pre-production
...
|---|---|---|---|
| **SEV-1 Critical** | Confirmed breach of Maaden data, or total loss of service | Data exfiltration; ransomware; auth bypass exploited in production | Immediate — `[XDH TO CONFIRM: e.g. 1 h to acknowledge, 24×7]` |
...
| Management Escalation | Legal/regulatory/commercial decisions | `[XDH TO CONFIRM]` |
...
**2 — Triage.** Incident Lead assigns severity within `[XDH TO CONFIRM]`. Start the incident
log immediately — every action, timestamped. The log is the evidence base for the report
and any regulatory position.

**3 — Contain.** Priority is stopping harm, not preserving uptime: revoke sessions/keys,
disable the affected path, isolate the workload. **Preserve evidence before remediating** —
snapshot logs and state first; do not "fix" the only copy of the proof.

**4 — Notify.** See §5.

**5 — Eradicate & recover.** Remove root cause, patch, restore from known-good backup where
needed, verify integrity before returning to service. Re-validate published asset data
integrity (payload hashes) if the publish path was implicated.

**6 — Post-incident review.** Blameless, within `[XDH TO CONFIRM — e.g. 5 business days]`.
Output: timeline, root cause, what worked, corrective actions with owners and dates. Feed
the corrective actions into the next audit cycle.

## 5. Notification

| Trigger | Notify | Timeline |
|---|---|---|
| SEV-1/SEV-2 affecting Maaden data or service | Maaden security contact | `[XDH TO CONFIRM — commit to a number, e.g. 24 h of confirmation]` |
| Confirmed personal-data breach | Maaden + regulator as applicable | `[XDH TO CONFIRM — must reflect Saudi PDPL obligations; take legal advice, do not guess]` |
| SEV-3/SEV-4 | Routine reporting | Next status report |

Notification content: what happened, what data/systems, when detected, what is being done,
what Maaden should do, next update time. **Do not speculate on cause in an early
notification** — state what is confirmed and what is still under investigation.

## 6. Evidence & retention

Incident logs, snapshots and reports retained for `[XDH TO CONFIRM]`. Audit trail data
...
**Review cycle:** `[XDH TO CONFIRM — e.g. annual, and after any SEV-1/SEV-2]`
